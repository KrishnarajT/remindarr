...
async def telegram_webhook(request: Request, db: Session = Depends(get_session)):
    ...
    if state.get("step") == 0:
        ...
        if cmd.startswith("http") or re.search(r"[0-9a-fA-F\-]{32,36}", cmd):
            # user directly sent a db url/id while in step 0
            state["step"] = 2
            notion_setup_states[chat_id] = state
            # fall through to step 2 handling below by reusing text
        ...
        # If we changed to step 2 above because user sent a db id, continue below

    # STEP 1: token submission (new token or change token)
    if state.get("step") == 1:
        ...
        try:
            ...
            db.commit()
            user_info = resp.json()
            notion_user_name = user_info.get("name") or user_info.get("owner", {}).get(
                "user", {}
            ).get("name", "your Notion account")
            send_message(
                settings.bot_token,
                chat_id,
